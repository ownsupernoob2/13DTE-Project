...
# Load scanner data from disk files
func get_scanner_data(disk_name: String) -> Dictionary:
	var scanner_data = {}
	var file_path = "res://assets/disks/" + disk_name.to_lower() + "_scanner_data.txt"
			...
			while not file.eof_reached():
				...
				if line.length() > 0 and ":" in line:
					...
					if parts.size() == 2:
						...
						scanner_data[key] = value
			file.close()
			print("Loaded scanner data for ", disk_name, ": ", scanner_data)
		else:
			print("Failed to open scanner data file: ", file_path)
	else:
		print("Scanner data file not found: ", file_path)
