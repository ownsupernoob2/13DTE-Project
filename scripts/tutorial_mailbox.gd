...
# Tutorial Mailbox interaction script for level_0.tscn
signal mail_taken
...
func _ready() -> void:
	...
	# Find the tutorial mail UI in the scene tree
	mail_ui = get_node("../TutorialMailUI")
	if not mail_ui:
		mail_ui = get_tree().get_first_node_in_group("mail_ui")
		if not mail_ui:
			print("Warning: Could not find TutorialMailUI node!")
	...
	print("Tutorial Mailbox ready - has mail: ", has_mail)
...
func take_mail() -> void:
	...
	is_interacting = true
	print("Taking mail from tutorial mailbox...")
...
func _show_mail_ui() -> void:
	...
	if mail_ui and mail_ui.has_method("show_mail"):
		mail_ui.show_mail()
		print("Tutorial mail UI displayed")
	else:
		# Fallback: print mail content to console
		print("Tutorial Mail Content: Welcome to Training Pod Alpha-7. Consult your Operations Manual and use the START control.")
...
func refill_mail() -> void:
	...
	print("Tutorial mail refilled and mailbox reset!")
